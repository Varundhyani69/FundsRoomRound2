...
- All ids in examples are 24-character lowercase hexadecimal strings. That is the stored form
  too: every primary key is a `CHAR(24)` hex string generated by `backend/src/db/id.js`, and
  the validation layer rejects anything not matching `/^[a-f0-9]{24}$/` with
...
| INVALID_REFERENCE    | 400         | `item` or `location` does not match an existing row |
| UNAUTHENTICATED      | 401         | No valid token supplied |
| FORBIDDEN            | 403         | Token role is not one of the three declared roles |
...
| INVALID_REFERENCE    | 400         | `location`, `item`, or `assignedUser` does not match an existing row |
| UNAUTHENTICATED      | 401         | No valid token supplied |
...
The Config_Loader (`backend/src/config/index.js`) reads exactly these variables at startup and
exits non-zero, naming every missing one, before the port is bound. Full setup detail,
including example values, lives in the README.

The MySQL settings are separate variables rather than one connection URL because that is the
shape a managed host hands you — AWS RDS presents endpoint, port, user and database name as
separate fields.

| Variable      | Purpose |
|----------------|----------|
| MYSQL_HOST     | Hostname or endpoint of the MySQL server |
| MYSQL_PORT     | TCP port of the MySQL server (1–65535) |
| MYSQL_USER     | MySQL user the API_Server connects as |
| MYSQL_DATABASE | Database (schema) holding the application's tables |
...
| MYSQL_PASSWORD | Password for MYSQL_USER. Must be present, but may be empty for a local passwordless install |

`SEED_ADMIN_PASSWORD`, `SEED_OPS_PASSWORD` and `SEED_SALES_PASSWORD` are deliberately absent
from this table: they are read by `scripts/seed.js` only, never by the API server.
